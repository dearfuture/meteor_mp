...
{    
	...
	if( con->buf[0] != SOCKS_VERSION_5){
		sys_log(LL_ERROR, "[ %s:%d ] error socks version: %d", __FILE__, __LINE__, con->buf[0] );
		close_session( process, con->session);
		return ;
	}
...
{    
	...
	socks_auth_reply_t reply;
	memset((void *)&req, 0, sizeof(req) ); 
	memset( (void *)&reply, 0, sizeof( reply ) );
	reply.status = SOCKS_AUTH_ERR_UNKOWN; // set default
...
{    
	...
	if (ret == SOCKS_CMD_SUCCESS )
	{
		...
		else if(cmd.cmd == SOCKS_COMMAND_UDP_ASSOCIATE)
		{
			con->session->protocol = SOCKS_PROTOCOL_UDP;
			_do_command_udp_associate( process, con, &cmd, &reply );
		}
		else
		{ //  unsupported
			reply.host.atype = SOCKS_ATYPE_IPV4;
